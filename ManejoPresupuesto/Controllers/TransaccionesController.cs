...
namespace ManejoPresupuesto.Controllers
{
    public class TransaccionesController : Controller
    {
        ...
        public async Task<IActionResult> Semanal(int mes, int año)
        {
            ...
            var diasSegmetados = diasDelMes.Chunk(7).ToList();

            for (int i = 0; i < diasSegmetados.Count(); i++)
            {
                var semana = i + 1;
                var fechaInicio = new DateTime(año, mes, diasSegmetados[i].First());
                var fechaFin = new DateTime(año, mes, diasSegmetados[i].Last());
